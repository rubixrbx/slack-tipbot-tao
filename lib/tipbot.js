...
var fs = require('fs');
var taod = require('bitcoin');
var texts = require('../text/txt_tao.js').tipbotTxt;
var sun = require('./sun');
var quiz = require('./quiz');
var waits = require('waitjs');
var tao = require('./dash');

const BLACKLIST_CURRENCIES = ['TAO'];

var TipBot = function (bot, RPC_USER, RPC_PASSWORD, RPC_PORT, OPTIONS) {
    var self = this;
    if (!bot) { throw new Error('Connection with Slack not availible for tipbot'); }

    self.HighBalanceWarningMark = tao.toDuff(2.0);
    ...
    self.OPTIONS = _.defaults(OPTIONS, {
        ...
        TX_FEE: tao.toDuff(0.0001), // TX fee, used in withdrawing, in Duffs
        WALLET_PASSW: null,
        ...
        SUN_DEFAULT_THRESHOLD: tao.toDuff(5) // duff
    });

    // create connection via RPC to wallet
    self.wallet = new taod.Client({
        host: 'localhost',
        port: RPC_PORT,
        user: RPC_USER,
        pass: RPC_PASSWORD,
    ...
    self.CURRENCY_REGEX = new RegExp('\\b(duff?|TAO|' + self.CURRENCIES.join('|') + ')\\b', 'ig'); // added \b: bugfix for only finding the currencr symbol instead parts of words (aud = audit)
...
TipBot.prototype.showPriceList = function (tellInChannel, all) {
    var self = this;
    var reply = { 'channel': tellInChannel.id, text: '' };
...
TipBot.prototype._getPriceRates = function (filename, cb) {
    fs.exists(filename, function (exists) {
        if (exists) {
            ...
            request.get('http://coinmarketcap-nexuist.rhcloud.com/api/tao/price', function (err, response, body) {
...
TipBot.prototype.getChannel = function (channelName, cb) {
    var self = this;
    var findNameExp = new RegExp(channelName + '(?![\\w-])', 'i'); // word boundaries that include the '-'
...
// return via callback amount in tao, and if it was needed to convertion rate and originalCurrency
...
TipBot.prototype.normalizeValue = function (inputValue, unit, user, cb) {
    ...
    if (inputValue === 'all' && user !== undefined) {
        user.getBalance(user.id, 6, function (err, balance) {
            ...
            var value = tao.toDuff(balance);
            debug('Log: using ALL balance of ' + user.name + ' = ' + balance);
            cb(value, null, null, null, tao.toTao(value) + ' TAO');
        });
        ...
        } else if (unit.match(/TAO/i)) {
            currency = 'TAO';
            value = tao.toDuff(inputValue);
        } else {
        ...
        self.getPriceRates(function (err, rates) {
            ...
            if (!rate) {
                ...
                var text = value.toFixed(2) + ' ' + currency + ' ' +
                    '(' + tao.toTao(newValue) + ' TAO at ' + rate.toFixed(2) + ' ' + currency + ' / TAO)';
                // return converted value in tao,  convertion rate, originalCurrency, originalValue, text about the convertion
                return cb(newValue, rate, currency, value, text);
            }
        });
    } else {
        // amount is in Tao, return only value, no convertion rate
        return cb(value, null, null, null, tao.toTao(value) + ' TAO');
    }
};

// tell all help text, if call by an admin show also the admin-only commands
TipBot.prototype.tellHelp = function (is_admin) {
    var text = _.reduce(texts.helpText, function (completeHelpText, helpPart) {
...
TipBot.prototype.showRandomHelp = function () {
    ...
    var helpCount = texts.helpText.length;
    var getHelpNR = Math.floor((Math.random() * helpCount));
    getHelpNR = getHelpNR === 0 ? 1 : getHelpNR; // don\'t show title
    var helpTxt = texts.helpText[getHelpNR].replace(/[*]/g, '').replace(/[_]/g, ''); // * for bold doesn't show in a code block
                            ...
                            self.getDirectMessageChannelID(null, oneUser.id, function (err, DMchannelRecievingUser) {
                                if (err === null) {
                                    ...
                                    var recievingUserMessage = {
                                        ...
                                        'text': texts.SunRecieved + tao.toTao(sunraySize) + ' tao'
                                    };
                                    self.slack.say(recievingUserMessage);
                                }
                            });
                        });
                        asyncCB();// callback needed to var async know everyhing is done
                    },
...
TipBot.prototype.onMessage = function (channel, member, message) {
    ...
    self.getDirectMessageChannelID(channel, user.id, function (err, DMchannelID) {
        ...
        if (message.match(/\bbalance\b/i)) {
            var balanceOfUser = user; // default show own balance (see balance check cmd)
        ...
        if (message.match(/\bwithdraw\b/i)) {
            ...
            if (providedCurrency !== null && providedCurrency[0].length !== 0) {
                ...
                //not provided, set tao as default currency
                amount[2] = 'TAO';
            }
            debug(amount);

            var address = message.match(self.ADDRESS_REGEX);
            ...
            self.normalizeValue(amount[1], amount[2], user, function (value, rate, originalCurrency, originalValue, valueText) {
                if (rate === false) {
                    ...
                    var privateConversation = { user: user.id };
                ...
                if (fromCurrency === 'tao') {
                    ...
                    self.normalizeValue(amount[1], toCurrency, user, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply.text = amount[1] + ' ' + fromCurrency + ' = ' + newValue.toPrecision(4) + '  ' + toCurrency +
                                ' ( 1.0 Tao = ' + rate.toPrecision(4) + ' ' + toCurrency + ' )';
                        }
                        self.slack.say(reply);
                    });
                } else if (toCurrency === 'tao') {
                    // other -> tao
                    self.normalizeValue(amount[1], fromCurrency, user, function (value, rate) {
                        if (rate === false) {
                            ...
                            reply.text = amount[1] + ' ' + fromCurrency + ' = ' + tao.toTao(value) + '  ' + toCurrency +
                                ' ( 1.0 ' + fromCurrency + ' = ' + (1.0 / rate).toPrecision(4) + ' TAO)';
                        }
                        self.slack.say(reply);
                    });
                }
            ...
            sun.getSunBalance(self.sunUser, function (err, sunBalance) {
                if (err) {
                    ...
                    if (sunBalance !== undefined && sunBalance > 2e-8) {
                        reply.text = texts.SunAvailibleAmount + sunBalance + ' tao';
                    } else {
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\bthreshold\b/i)) {
                    ...
                    if (amount !== null) {
                        ...
                        sun.saveThreshold(tao.toDuff(amount[1]),
                            function (err) {
                                if (err) {
                                    debug(err); return;
                                } else {
...
/*
        ...
        if (message.match(/\bquiz\b/i)) {
            ...
            if (message.match(/\banswer\b/i)) {
                var answer = message.replace('answer', '').replace('quiz', '').trim();
                            ...
                            if (done) {
                                ...
                                quiz.end(function (scores) {
                                    ...
                                    _.forEach(scores, (score, username) => {
                                        var scoreText = username + texts.QuizScored + score + '\n';
                                        debug(scoreText);
            ...
            if (message.match(/\badd\b/i)) {
                ...
                var askConfirmation = function (question, answer, convo) {
                    convo.ask(
                ...
                // ask for answer
                var askAnswer = function (question, convo) {
                ...
                // ask for question
                var askQuestion = function (err, convo) {
                    if (err) {
            ...
            if (user.is_admin) {
                ...
                if (message.match(/\blist\b/i)) {
                    quiz.getApprovedQuestions(function (err, questions) {
                        ...
                        if (questions.length === 0) {
                            ...
                            var questionsSentence = texts.QuizListQuestions;
                ...
                if (message.match(/\breview\b/i)) {
                    quiz.getUnreviewedQuestions(function (err, questions) {
                        ...
                        if (questions.length === 0) {
                            ...
                            var questionsSentence = texts.QuizUnreviewed;
                ...
                if (message.match(/\bdelete\b/i)) {
                    var questionNumber = message.match(/\d+/i);
                ...
                if (message.match(/\breward\b/i)) {
                    ...
                    quiz.getQuestion(questionNumber, function (err, question) {
                        ...
                        var privateConversation = { user: user.id };
                        self.slack.startPrivateConversation(privateConversation, function (err, convo) {
                            var askSentence = '_' + question.question + '_\n' + texts.QuizSetReward;
                            convo.ask(askSentence, function (response, convo) {
                                var reward = parseFloat(response.text);
                                if (reward === undefined) {
...
*/
        //  * OOPS
        var amountOfPossibleResponds = texts.NoCommandFound.length;
