'use strict';
var debug = require('debug')('tipbot:sun');
var helpTexts = require('../text/txt_tao.js').tipbotTxt;
...
var init = (sunUserName, users) => {
    var sunUser = null;
    // get tipbot user that hold the sun/rain balance
    var findSunUser = _.filter(users,
        function (match) {
...
var getAmountOfEligibleSunUsers = cb => {
    Tipper.count(
        { gotSunshine: false },
        (err, amountOfTippers) => {
...
function shineNow(sunUser, sunBalance, cb) {
    if (sunUser === undefined || sunUser === null) {
        cb('ERROR sun: cannot var is sun as sun User is unknown !');
        return;
    }
    ...
    getSunRaySize(sunBalance, function (err, sunraySize) {
        ...
        getListOfSunEligibleUsers(function (err, usersList) {
            async.forEachSeries(usersList,
                (oneUser, asyncCB) => {
                    debug('Cast a sunray of ' + tao.toTao(sunraySize) + ' tao on ' + oneUser.name + ' (' + oneUser.id + ')');
...
var checkThreshold = (defaultThreshold, sunUser, cb) => {
    getThreshold(defaultThreshold,
        (err, threshold) => {
            if (err) { cb(err); return; }
            getSunBalance(sunUser,
                function (err, sunBalance) {
                    if (tao.toDuff(sunBalance) >= threshold) {
